...
function loadOAuth() {
    ...
    function initClient() {
        gapi.client.init({
            ...
            showNotification('toast', "Error in OAuth:" + ex, 'error');
        });
    }
    /**
...
function search(o, defer) {
    var request = gapi.client.youtube.search.list({
        q: o.tag,
        part: 'snippet'
    });
    request.execute(function (response) {
        if (response.result && response.result.items[0].id.videoId) {
            o["VID_ID"] = response.result.items[0].id.videoId;
            o["VID_LIST"] = response.result.items;
...
function onShowModalListMoreMusic(e) {
    if (e && e.target) {
        ...
        list = list.dataSource.get(id).videoList, promise = [];
        for (var i = 0; i < list.length; i++) {
            var d = $.Deferred();
            promise.push(d);
            list[i]["Autor"] = list[i].snippet.title;
            list[i]["Titulo"] = "";
            list[i]["lenght"] = "";
            search(list[i], d);
        }
    ...
    if (!obs_DataSheetOptions) {
        obs_DataSheetOptions = new kendo.observable({
            ...
            open_MusicDataSheet: function (e) {
                ...
                if (this.file_Id) {
                    getDataSheetColumns(this.file_Id, "SELECT A, B LIMIT 5").then(function (rsp) {
